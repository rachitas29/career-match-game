...
async function loadLineItemData() {
    ...
    try {
        ...
        // Check if the PO exists in the database (for display purposes)
        ...
        if (poRes.purchase_order) {
            ...
            console.log('[LineItems] PO does not exist in database yet (new PO)');
            lineItemsState.poExistsInDB = false;
            lineItemsState.currentPO = null;
        }

        // Fetch line items (may be empty for new POs)
    ...
    lineItemsState.currentLineItems.forEach((li, idx) => {
        ...
        li.milestones.forEach((ms, msIdx) => {
            ...
            tr.innerHTML = `
                ...
                <td class="col-milestone">${ms.milestone_name}</td>
                <td class="col-value">${ms.cycle_value}</td>
...
async function saveBillingItem() {
    ...
    // --- START INVOICE SPLITTING LOGIC ---
    const newInvNo = document.getElementById('bill_invoice_no').value.trim();
    const rows = document.querySelectorAll('#billingGridBody tr');
